            ...
            <main class="<?php echo $rounded; ?>">
                        ...
                        <div class="header-img border-top">
                            <?php if ($this->options->headerImageProportion == 'not-fixed' or $this->options->headerImageProportion == 'post-page-fixed'): ?>
                                <a target="<?php $this->options->listLinkOpen(); ?>" href="<?php $this->permalink(); ?>">
                                    <img src="<?php echo $img; ?>" alt="<?php $this->title(); ?>的頭圖" style="background-color: <?php echo headerImageBgColor($this->options->headerImageBg); ?>;">
                                </a>
                            <?php else: ?>
                                <a tabindex="-1" aria-hidden="true" href="<?php $this->permalink() ?>" aria-label="<?php $this->title() ?>的頭圖" style="background-image: url(<?php echo $img; ?>);background-color: <?php echo headerImageBgColor($this->options->headerImageBg); ?>;" class="fixed"></a>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
                <div class="article-info clearfix border-bottom border-top" role="group" aria-label="文章信息">
                    ...
                    <div class="info">
                        ...
                        <span class="<?php echo $color['link']; ?>" data-toggle="tooltip" data-placement="top" tabindex="0" title="發佈日期：<?php $this->date('Y年m月d日'); ?>"><?php $this->date('Y年m月d日'); ?></span>
                    </div>
                    <!--作者-->
                    <div class="info">
                    ...
                    <div class="info">
                        ...
                        <span class="<?php echo $color['link']; ?>" data-toggle="tooltip" data-placement="top" tabindex="0" title="閱讀量：<?php echo getPostView($this); ?>"><?php echo getPostView($this); ?></span>
                    </div>
                    <!--评论-->
                    <div class="info">
                        <i class="icon-bubbles2 icon <?php echo $color['link']; ?>" aria-hidden="true"></i>
                        <a class="<?php echo $color['link']; ?>" data-toggle="tooltip" data-placement="top" title="評論" href="<?php $this->permalink() ?>#comments"><?php $this->commentsNum('%d 評論'); ?></a>
                    </div>
                    <!--分类-->
                    ...
                    <div class="info tags" data-color="<?php echo $color['link']; ?>">
                        ...
                        <?php $this->tags(' ', true, '暫無標籤'); ?>
                    </div>
                    <?php if ($this->user->hasLogin()): ?>
                        <div class="info d-sm-none d-none d-md-inline d-lg-inline d-xl-inline">
                            <i class="icon icon-pencil <?php echo $color['link']; ?>"></i>
                            <a class="<?php echo $color['link']; ?>" href="<?php echo $this->options->siteUrl . 'admin/write-post.php?cid=' . $this->cid; ?>" >編輯</a>
                        </div>
                    <?php endif; ?>
                </div>
                <!--文章内容-->
                <article>
                    ...
                    <div class="clearfix">
                        <?php if ($this->options->modified == 'show'): ?>
                            <span class="float-xl-left float-lg-left float-md-left d-block" data-toggle="tooltip" data-placement="top" tabindex="0" title="發佈時間：<?php $this->date('Y年m月d日'); ?>">最後編輯：<?php echo date('Y年m月d日', $this->modified);?></span>
                        <?php endif; ?>
                        <?php if ($this->fields->articleCopyright != 'hide'): ?>
                            <span tabindex="0" data-toggle="tooltip" data-placement="top" title="本文爲原創文章，版權歸 <?php $this->options->title(); ?> 所有，轉載請聯繫博主獲得授權。" class="mt-1 mt-sm-1 mt-md-0 mt-lg-0 mt-lg-0 mt-xl-0 float-xl-right float-lg-right float-md-right d-block">©著作權歸作者所有</span>
                        <?php endif; ?>
                    </div>
                ...
                <nav class="post-navigation navbar border-top row">
                    ...
                    <div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 next">
                        ...
                        <div class="text-lg-right text-xl-right text-md-right next-box"><?php $this->theNext('%s','沒有了'); ?></div>
                    </div>
                </nav>
...
<div id="max-img" role="dialog">
    ...
    <div class="btn-group" role="group" aria-label="圖片工具欄" id="img-control">
        <button type="button" class="btn btn-dark big" title="放大" aria-label="放大">
            <i class="icon-zoom-in"></i>
        </button>
        <button type="button" class="btn btn-dark small" title="縮小" aria-label="縮小">
            <i class="icon-zoom-out"></i>
        </button>
        <button type="button" class="btn btn-dark spin-left" title="左旋轉90度" aria-label="左旋轉90度">
            <i class="icon-undo"></i>
        </button>
        <button type="button" class="btn btn-dark spin-right" title="右旋轉90度" aria-label="右旋轉90度">
            <i class="icon-redo"></i>
        </button>
        <button type="button" class="btn btn-dark hide-img" title="關閉大圖（ESC）" aria-label="關閉大圖（ESC）">
